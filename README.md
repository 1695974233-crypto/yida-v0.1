...
国内预发布地址：[易搭 v0.1（火山引擎北京区）](https://s877em6sdkp7rquqv523o.apigateway-cn-beijing.volceapi.com/)
...
- 12 件基础单品组成的虚拟衣柜。
- 用户独立、可持久化的衣柜数据。
- 邮箱注册/登录、Google 登录和 GitHub 登录。
- 上衣、下装、外套和鞋子的组合推荐。
- 天气、场景、风格、保暖度和衣服可用状态评分。
- 用户主动定位或手动搜索城市，读取真实温度、体感、降雨和风速。
- 上班、约会、休闲和运动场景快捷选择。
- 对话式穿搭需求输入。
...
- 连衣裙可以独立与鞋子、外套组成推荐。
- 公开体验期每位访客每天最多识别 20 次，按北京时间每日重置。
- AI 模特效果图按需生成，普通用户每天最多成功生成 10 次，开发者账号免额度。
- 模型未配置或调用失败时的安全回退。

## v0.1 尚未实现

- 天气服务商业化方案评估；当前 MVP 使用 Open-Meteo 原型服务。
- 生产级自动抠图与颜色校正，目前 Seedream 只作为可选展示图整理。
- 重复衣物识别和批量录入。
- 大模型驱动的自由对话。
- 更高保真、速度更稳定的真人虚拟试穿专用模型。
- 抖音式个性化信息流模型。
- 生产级内容版权、审核和合规流程。
...
| [veFaaS 迁移说明](docs/vefaas-migration.md) | 国内预发布架构、部署方式、验收结果与后续域名步骤 |
...
- Supabase Postgres + 行级访问控制（RLS）
- Supabase Storage 私有图片桶
- OpenAI Sites 旧版公开体验部署（迁移期保留）
- 火山方舟中国区模型适配

大模型自由对话将在后续版本接入。天气数据由 [Open-Meteo](https://open-meteo.com/) 提供。

注意：当前计算与访问入口位于火山引擎北京区，但 Supabase 项目仍在美国区域，因此这不是“全部数据境内存储”的最终合规架构。
